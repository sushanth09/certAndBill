...
(function() {
  ...
  angular.module('pdf', []).directive('ngPdf', [ '$window', function($window) {
    ...
    var setCanvasDimensions = function(canvas, w, h) {
	  ...
	  canvas.setAttribute('class', 'cAlign');
      return canvas;
    };
    return {
      restrict: 'E',
      ...
      link: function(scope, element, attrs) {
        ...
        scope.goPrevious = function(isCallFromMapping, pageNumber) {
        	if(scope.virtualNextShow){
            	scope.navigateDocumentPrev(scope.documentMap);
          }else{
          if (scope.pageToDisplay <= 1) {
            return;
          } else if(isCallFromMapping) {
            scope.pageToDisplay = pageNumber;
          } else {
        ...
        scope.goNext = function(isCallFromMapping, pageNumber) {
        if(scope.virtualNextShow){
        	scope.navigateDocumentNext(scope.documentMap);
        }else{
	          if (pdfDoc && scope.pageToDisplay >= pdfDoc.numPages) {
	            return;
	          } else if(isCallFromMapping) {
	            scope.pageToDisplay = pageNumber;
	          } else {
        ...
        scope.rotate = function() {
          if (canvas.getAttribute('class') === 'rotate0 cAlign') {
            canvas.setAttribute('class', 'rotate90 cAlign');
          } else if (canvas.getAttribute('class') === 'rotate90 cAlign') {
            canvas.setAttribute('class', 'rotate180 cAlign');
          } else if (canvas.getAttribute('class') === 'rotate180 cAlign') {
            canvas.setAttribute('class', 'rotate270 cAlign');
          } else {
            canvas.setAttribute('class', 'rotate0 cAlign');
          }
        };
        ...
        scope.$watch('pdfUrl', function(newVal) {
          if (newVal !== '') {
            ...
            url = newVal;
            scope.pageNum = scope.pageToDisplay > 0 ? scope.pageToDisplay :  pageToDisplay;
           /* if (pdfLoaderTask) {
