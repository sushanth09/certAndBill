		...
		function initDom() {
			...
			if (options.showToolbar) {
				...
				$element.find(".pdf-toolbar").html(
					...
					'<div class="pages"><span class="pdf-page-count"></span></div>'
				 	+ '<div class="pdf-button"><button class="pdf-prev pdf-next-pre"></button></div>'
				 	+ '<div class="pdf-button"><button class="pdf-next pdf-next-pre"></button></div>'
				 	+ (options.disableZoom? '':'<div class="buttonsPDF"><button class="pdf-zoomin zoom-in"></button>'
				 	+'<button class="pdf-zoomreset reset"></button>'
				 		+ '<button class="pdf-zoomout zoom-out"></button></div>')
					);
          ...
          $drag.panzoom('enable');
				...
				if (options.showToolbar) {
					$element.find(".pdf-toolbar > .buttonsPDF > .pdf-zoomin").on("click", function() {
						zoomIn();
					});
					$element.find(".pdf-toolbar > .buttonsPDF > .pdf-zoomout").on("click", function() {
						zoomOut();
					});
					$element.find(".pdf-toolbar > .buttonsPDF > .pdf-zoomreset").on("click", function() {
						zoomReset();
					});
				}
		...
		*/
		...
		function redraw() {
		  var pdfHeight, pdfWidth;
			...
			if (state == INIT) {
				 pdfHeight = options.loadingHeight;
				 pdfWidth = options.loadingWidth;

			} else {
				 pdfHeight = canvas.height / options.quality;
				 pdfWidth = canvas.width / options.quality;
			}
			...
			$element.find(".pdf-outerdiv")
				...
				.css("height", pdfHeight + TOOLBAR_HEIGHT + BORDER_WIDTH*2 - 40)
		...
		function updatePageCount() {
			if (state == EMPTY || state == INIT) return;
			$element.find(".pdf-page-count").html("<input type='text' value='" + pageNum + "' />" + " / " + totalPages);
		}
